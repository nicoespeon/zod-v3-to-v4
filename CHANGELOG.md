...
- Migrate `ZodType<Output, Def, Input>` generics to `ZodType<Output, Input>` (drops the middle `Def` parameter)
- Migrate `z.ostring()`, `z.onumber()`, `z.oboolean()` to `z.string().optional()`, `z.number().optional()`, `z.boolean().optional()`
- Migrate `._def` access to `._zod.def`
...
- Make `zod` an actual dependency instead of a dev dependency. This issue was introduced with v1.11.0 and could prevent the codemod from running correctly. [#83](https://github.com/nicoespeon/zod-v3-to-v4/issues/83)
...
[Unreleased]: https://github.com/nicoespeon/zod-v3-to-v4/compare/1.17.0...HEAD
